...
/**
 * This class is designed to simulate a high CPU load scenario. It contains methods to start and
 * stop a specified number of worker threads designed to perform CPU-intensive calculations.
 */
public class CPULoad {
  ...
  /**
   * Starts or stops the CPU load generation based on the input parameter. If enabled, it launches
   * worker threads. If disabled, it stops any running threads.
   *
  ...
  private static class Logarithmizer implements Runnable {
    ...
    /** Continuously calculates the logarithm of the current system time until requested to stop. */
    @Override
    public void run() {
      while (shouldRun) {
    ...
    /**
     * Sets the shouldRun flag to control whether this Logarithmizer should continue to run.
     *
